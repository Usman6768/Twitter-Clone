...
export const likeUnlikePost = async (req,res) => {
    try {
        ...
        const userLikedPost = post.likes.includes(userId); // check if user has liked the post
        ...
        if(userLikedPost){
            ...
            const updatedLikes = post.likes.filter((id) => id.toString() !== userId.toString()) // remove user id from likes array
...
export const getLikedPosts = async (req,res) => {
    ...
    try {
        ...
        .populate({
         ...
         // find posts that are in the likedPosts array of the user
...
export const getFollowingPosts = async(req,res) => {
    try {
        ...
        const feedPosts = await Post.find(
            {user :{$in: following}} // find posts where the user is in the following array of the user
            ).sort({createdAt: -1})
            .populate({
                path: "user",
                select: "-password"
