...
export const followUnfollowUser = async (req,res) => {
    try {
        const {id} = req.params
        const userToModify = await User.findById(id) // the user we have to follow or unfollow
        ...
        const isFollowing = currentUser.following.includes(id)
         ...
         if(isFollowing){
            ...
            res.status(200).json({message: "User unfollowed Succussfully"})
         }
         else{
            // Follow the user
            ...
            res.status(200).json({message: "User followed Succussfully"})
         }
